...
RSpec.describe "World Clock candidate reviews", type: :request do
  ...
  it "validates candidate instants without creating an Event" do
    expect {
      post world_clock_candidate_review_path, params: { instants: future_times }
    }.not_to change(Event, :count)

    expect(response).to have_http_status(:ok)
    expect(response.parsed_body.fetch("instants").length).to eq(2)
  end
  ...
  it "returns validation errors for invalid candidates" do
    ...
    expect(response.parsed_body.fetch("errors")).to include(/between 2 and 10/)
  end
end
